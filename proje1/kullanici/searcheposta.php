...
if(!empty($_POST["search"])) {
    ...
    $search=$connection->prepare(" SELECT * FROM  uyeler WHERE eposta LIKE ? ");
    $search->execute(array('%'.$key.'%'));
}
?>

<form action="" method="post" name="search">
    <input type="text"  name="search" placeholder="eposta arama">
...
<?php if (empty($key)) {}
    ...
    <?php foreach ($search as $data): ?>
        ...
        <br>    <a href="profil.php?id=<?= $data['id'] ?>"> profiline git </a><br><hr>
    <?php endforeach; ?>
<?php } ?>
